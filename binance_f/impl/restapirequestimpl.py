...
class RestApiRequestImpl(object):
    ...
    def priceIndex(self, symbol):
        builder = UrlParamsBuilder()
        builder.put_url("symbol", symbol)
        request = self.__create_request_by_get_with_signature("/sapi/v1/margin/priceIndex", builder)
        def parse(json_wrapper):
            return json_wrapper
        request.json_parser = parse
        return request
    
    def crossMarginCollateralRatio(self):
        builder = UrlParamsBuilder()
        request = self.__create_request_by_get_with_signature("/sapi/v1/margin/crossMarginCollateralRatio", builder)
        def parse(json_wrapper):
            return json_wrapper
        request.json_parser = parse
        return request
